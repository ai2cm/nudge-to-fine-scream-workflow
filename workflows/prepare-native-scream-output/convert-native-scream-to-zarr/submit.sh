...
python /usr/workspace/climdat/eamxx-ml/fv3net/projects/scream/scream_post/format_output.py "$scream_output_files" $output_zarr_path T_mid,qv,U,V physics,nudging 12 --calc-physics-tend=True --rename-nudging-tend=True --rename-delp=True --convert-to-cftime=True --rename-water-vapor-path=True --rename-lev-to-z=True --split-horiz-winds-tend=True --add-rad-fluxes=True --add-phis=True
